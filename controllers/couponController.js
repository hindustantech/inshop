  ...
  try {
    ...
    const commonPipeline = [
      ...
      ...(search.trim()
        ...
        : []),
      // Early filter for active and valid coupons to reduce documents before lookup or other stages
      {
        $match: {
          active: true,
          $or: [
      ...
      // Early projection to reduce fields carried through the pipeline
      {
        $project: {
          title: 1,
          shop_name: 1,
    ...
    if (userId) {
      facetPipeline = [
        ...
        { $unwind: { path: '$userStatus', preserveNullAndEmptyArrays: true } },
        // Filter based on userStatus after lookup
        {
          $match: {
            $or: [
              { userStatus: { $exists: false } }, // Unclaimed coupons
              {
                $and: [
                  { 'userStatus.status': { $nin: ['used', 'transferred'] } },
        ...
        // Use $facet to compute data and count in one aggregation
        {
          $facet: {
            data: [
              {
                $addFields: {
                  couponCount: {
                    $cond: {
                      if: { $and: [{ $ne: ['$userStatus', null] }, { $gte: ['$userStatus.count', 1] }] },
                      then: '$userStatus.count',
                      else: 1, // Unclaimed coupons have a count of 1
                    },
                  },
                  baseDisplayTag: {
                    $switch: {
                      ...
                      default: 'Available', // Default for unclaimed or invalid status
                    },
                  },
                },
              },
              ...
              // Unwind the repeatArray to duplicate documents
              { $unwind: { path: '$repeatArray', includeArrayIndex: 'instanceIndex' } },
              // Add displayTag with instance number (starting from 1)
              {
                $addFields: {
                  displayTag: {
                    $cond: {
                      if: { $eq: ['$baseDisplayTag', 'Available'] },
                      then: {
                        $concat: [
                          'Available ',
                          { $toString: { $add: ['$instanceIndex', 1] } },
                        ],
                      },
                      else: '$baseDisplayTag', // Keep 'Cancelled' or other statuses unchanged
                    },
                  },
                  distanceInKm: { $round: [{ $divide: ['$distance', 1000] }, 2] },
                },
              },
              // Final project for data
              {
                $project: {
                  _id: 1,
                  title: 1,
                  ...
                  total: {
                    $sum: {
                      $cond: {
                        if: { $and: [{ $ne: ['$userStatus', null] }, { $gte: ['$userStatus.count', 1] }] },
                        then: '$userStatus.count',
                        else: 1, // Unclaimed coupons contribute 1 to the count
                      },
                    },
                  },
                },
        ...
        ...commonPipeline,
        {
          $facet: {
            data: [
              {
                $addFields: {
                  displayTag: { $ifNull: [{ $arrayElemAt: ['$tag', 0] }, 'Available 1'] }, // Use 'Available 1' for consistency
                  couponCount: 1,
                  distanceInKm: { $round: [{ $divide: ['$distance', 1000] }, 2] },
                },
              },
    ...
    // Execute the single aggregation with $facet
    const result = await Coupon.aggregate(facetPipeline);
    const coupons = result[0]?.data || [];
    const total = result[0]?.total[0]?.total || 0;
    // Calculate uniqueCoupons
    const uniqueCoupons = coupons.length > 0 ? [...new Set(coupons.map(c => c._id.toString()))].length : 0;
    res.status(200).json({
      success: true,
      mode,
      data: coupons,
